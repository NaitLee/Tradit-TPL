...
new Macro('replace', c => {
    ...
    let args: string[] = [];
    const pass = c.r;
    let value1: string, value2: string;
    while (c.p.isNotEmpty()) {
        value1 = c.p.nextString(), value2 = c.p.nextString();
        if (s_result !== NULL_STRING && value1 !== NULL_STRING && value2 !== NULL_STRING) {
            s_result = s_result.replace(value1, value2);
        } else {
            args.push(value1, value2);
        }
    }
    if (s_result !== NULL_STRING && args.length === 0) {
        if (pass) return EchoPass(s_result);
        c.q.pushString(s_result);
        return null;
    }
    return async d => {
        let result = s_result === NULL_STRING ? d.s.nextString(true) : s_result;
        for (let i = 0; i < args.length; i += 2) {
            result = result.replace(
                args[i] === NULL_STRING ? d.s.nextString() : args[i],
                args[i + 1] === NULL_STRING ? d.s.nextString() : args[i + 1]
            );
        }
